...
import (
  http "net/http"
  json "encoding/json"
  countryService "country-service/services"
...
func Save(writer http.ResponseWriter, request *http.Request) {
  ...
  code, err := countryService.Save(country)
  if err != nil {
    if err = json.NewEncoder(writer).Encode(err); err != nil {
  			panic(err)
  		}
...
func FindOne(writer http.ResponseWriter, request *http.Request) {
  ...
  country, err := countryService.FindOne(code)
  if err != nil {
    if err = json.NewEncoder(writer).Encode(err); err != nil {
  			panic(err)
  		}
  ...
  writer.Header().Set("Content-Type", "application/json; charset=UTF-8")
	writer.WriteHeader(http.StatusOK)
  if err = json.NewEncoder(writer).Encode(country); err != nil {
			panic(err)
		}
}
